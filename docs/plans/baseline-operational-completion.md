...
`StandardSchemaValidationPipe` remains a target strategy name in the architecture documents, not a
verified source export or installed runtime API. Likewise, the OpenAPI owner requires
`@nestjs/swagger`, but it is not present in `package.json` and no Swagger bootstrap is verified. Do
not treat those names as implementation-ready APIs.
