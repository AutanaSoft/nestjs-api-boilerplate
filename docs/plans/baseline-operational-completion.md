...
- Typed Zod configuration factories registered with `registerAs` provide `app`, `api`, `http`,
  `cors`, `rateLimit`, and `openapi` (`src/config/`;
...
- [ ] All unresolved implementation decisions, including the canonical pnpm version/declaration, are
      resolved and reflected in the relevant owners.
