...
The application needs a first business capability that owns user data and supports basic user
management before authentication and authorization are introduced. This PDR defines the approved
minimum product boundary and its transition to protected access.

## 1. Problem

The application has no business-owned representation or management lifecycle for users. Without that
foundation, later authentication and authorization capabilities would have no stable user API to
depend on and could become coupled directly to persistence concerns.

## 2. Goal

Provide a minimum user management capability that creates, retrieves, lists, queries, partially
updates, and deletes users through a stable public contract.
...
- Experimentally query users with approved structured criteria through a safe and idempotent
  operation without replacing conventional collection retrieval.
...
- **Public user identifier:** A UUIDv4 generated natively by PostgreSQL 16 and exposed as the
  stable, immutable user identifier.
- **Email:** A required, non-nullable user attribute. Leading and trailing whitespace is removed,
  the complete value is lowercased, and format validation occurs after normalization. Uniqueness is
  enforced on the normalized value. Dots and `+tag` suffixes are preserved.
- **Display name:** A required, non-nullable human-readable name. Surrounding whitespace is removed,
  length must be between 2 and 30 characters, and only Unicode letters and internal spaces are
  accepted. Numbers and symbols are rejected.
- **Creation timestamp:** `createdAt` is generated by the system when the user is created and
  exposed in the public representation.
- **Update timestamp:** `updatedAt` is managed by the system when the user changes and exposed in
  the public representation.
- **Public user representation:** The approved set of fields that may be returned to API consumers.
- **Conventional user filters:** `GET` supports exact matching on normalized `email`. `displayName`,
...
- **Deletion policy:** Users are physically removed. Deleted users cannot be retrieved or restored,
  and no `deletedAt` field is retained.
- **Successful responses:** Creation returns `201 Created`, the created user, and `Location`; reads
  and partial updates return `200 OK` with their user or paginated collection representation;
  deletion returns `204 No Content` without a body. Every response includes `X-Request-Id` through
  the shared HTTP contract.
- **Authentication transition:** When `AuthModule` is implemented, every Users management operation
  requires authentication and explicit authorization. Registration belongs to `AuthModule`; no
  administrative Users operation remains public.

The approved initial public model is `id`, `email`, `displayName`, `createdAt`, and `updatedAt`.
Client input cannot assign or modify the identifier or timestamps.

## 5. Implementation notes

- Preserve `UsersModule` as the owner of user behavior and keep future authentication behavior
  dependent on its public API.
- Follow the existing repository conventions for HTTP contracts, pagination, validation,
  serialization, errors, persistence, OpenAPI, and testing rather than redefining them here.
- Replace the temporary unauthenticated exposure with authentication and explicit authorization when
  `AuthModule` is introduced.

## 6. Acceptance criteria

- [ ] A consumer can create a user only with a required email that is trimmed, lowercased, and
      validated before persistence.
- [ ] A consumer can create a user only with a required display name containing 2–30 Unicode letters
      and internal spaces after trimming surrounding whitespace.
- [ ] A successful creation returns `201 Created`, the public user representation, and a `Location`
      header identifying the new resource.
- [ ] A successful creation assigns a UUIDv4 and both timestamps through the system and returns only
      the approved public user representation.
...
- [ ] `GET /users` rejects unsupported filters, including `displayName` and date filters in the
      initial contract.
- [ ] User listing defaults to `createdAt desc`.
- [ ] A consumer can sort user listings by `createdAt` or `displayName` in ascending or descending
      order.
...
- [ ] Experimental `QUERY` returns `200 OK` and uses the same paginated collection, pagination, and
      sorting contracts as conventional `GET`.
- [ ] Repeating the same structured query against unchanged data produces equivalent results without
      modifying user data.
...
- [ ] A consumer can physically delete a user, after which retrieving that identifier returns
...
- [ ] When `AuthModule` is introduced, every Users management operation requires authentication and
      explicit authorization, while registration is owned by `AuthModule`.
...
- Require a non-nullable email, trim surrounding whitespace, lowercase the complete value, validate
  the normalized result, and enforce uniqueness on that result.
- Preserve dots and `+tag` suffixes during email normalization.
- Require a non-nullable display name, trim surrounding whitespace, enforce a 2–30 character length,
  and accept only Unicode letters and internal spaces.
- Include system-managed `createdAt` and `updatedAt` values in both persistence and the public user
  representation.
...
- Limit experimental `QUERY` to exact normalized email, using the same pagination and sorting as
  `GET` without adding filter fields or operators.
...
- Require authentication and explicit authorization for every Users management operation when
  `AuthModule` is implemented; keep registration under `AuthModule`.

### Considered options

- Use `GET` for conventional collection queries and evaluate experimental `QUERY` only for safe,
  structured queries requiring request content.
- Start from a minimal public model containing `id`, `email`, `displayName`, `createdAt`, and
  `updatedAt`.
